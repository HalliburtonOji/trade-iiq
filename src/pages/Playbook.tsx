  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {showForm && (
          <div style={{ background: "var(--stoa-shine)", border: "1px solid var(--stoa-rule)", borderRadius: 2, padding: 14 }} className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="stoa-kicker">{editingId ? "EDIT" : "CREATE"} · Γραφή</span>
              <button onClick={resetForm}><X className="h-4 w-4" style={{ color: "var(--stoa-muted)" }} /></button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Name</label>
                <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Bull Flag Breakout" />
              </div>
              <div className="space-y-2">
                <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Strategy Type</label>
            ...
            <div className="space-y-2">
              <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Checklist</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Entry Conditions</label>
                <Textarea value={entryCondition} onChange={e => setEntryCondition(e.target.value)} placeholder="When to enter..." rows={3} />
              </div>
              <div className="space-y-2">
                <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Exit Conditions</label>
                <Textarea value={exitCondition} onChange={e => setExitCondition(e.target.value)} placeholder="When to exit..." rows={3} />
              </div>
            </div>

            <div className="space-y-2">
              <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Invalidation Rules</label>
            ...
            <div className="space-y-2">
              <label className="stoa-kicker" style={{ color: "var(--stoa-muted)" }}>Notes</label>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {playbooks.map(p => (
              <div
                key={p.id}
                style={{ background: "var(--stoa-shine)", border: "1px solid var(--stoa-rule)", borderLeft: "3px solid var(--stoa-accent)", borderRadius: 2, padding: 14 }}
                className="space-y-2"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 cursor-pointer flex-1" onClick={() => setExpandedId(expandedId === p.id ? null : p.id)}>
                    <BookOpen className="h-4 w-4" style={{ color: "var(--stoa-accent)" }} />
                    <div>
                      <h3 className="stoa-display" style={{ fontSize: 14, color: "var(--stoa-ink)" }}>{p.name}</h3>
                      <Badge variant="secondary" className="text-[10px] mt-0.5 stoa-kicker">{p.strategy_type}</Badge>
                    </div>
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => startEdit(p)} className="p-1.5 rounded-lg" style={{ borderRadius: 2 }}><Edit2 className="h-3.5 w-3.5" style={{ color: "var(--stoa-muted)" }} /></button>
                    <button onClick={() => deletePlaybook(p.id)} className="p-1.5 rounded-lg" style={{ borderRadius: 2 }}><Trash2 className="h-3.5 w-3.5 text-destructive" /></button>
                  </div>
                </div>
                {expandedId === p.id && (
                  <div className="pt-2 space-y-3 text-sm" style={{ borderTop: "1px solid var(--stoa-rule)" }}>
                    {p.checklist.length > 0 && (
                      <div>
                        <p className="stoa-kicker mb-1" style={{ color: "var(--stoa-muted)" }}>Checklist</p>
                        <ul className="space-y-1">
                          {p.checklist.map((c, i) => (
                            <li key={i} className="flex items-center gap-2" style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "var(--stoa-ink)" }}>
                              <Check className="h-3 w-3" style={{ color: "var(--stoa-accent)" }} />{c}
                            </li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {(p.conditions.entry || p.conditions.exit) && (
                      <div className="grid grid-cols-2 gap-3">
                        {p.conditions.entry && <div><p className="stoa-kicker mb-1" style={{ color: "var(--stoa-muted)" }}>Entry</p><p style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "var(--stoa-ink)" }}>{p.conditions.entry}</p></div>}
                        {p.conditions.exit && <div><p className="stoa-kicker mb-1" style={{ color: "var(--stoa-muted)" }}>Exit</p><p style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "var(--stoa-ink)" }}>{p.conditions.exit}</p></div>}
                      </div>
                    )}
                    {p.invalidation_rules && <div><p className="stoa-kicker mb-1" style={{ color: "var(--stoa-muted)" }}>Invalidation</p><p style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "var(--stoa-ink)" }}>{p.invalidation_rules}</p></div>}
                    {p.notes && <div><p className="stoa-kicker mb-1" style={{ color: "var(--stoa-muted)" }}>Notes</p><p style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "var(--stoa-ink)" }}>{p.notes}</p></div>}
                  </div>
                )}
              </div>
            ))}
