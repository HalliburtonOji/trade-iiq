...
const MentorPulse = ({ profile, lastJournalAt }: Props) => {
  ...
  useEffect(() => { if (!lastJournalAt) return; setPulse(true); const t = setTimeout(() => setPulse(false), 2400); return () => clearTimeout(t); }, [lastJournalAt]);
  ...
  return (
    <div
      className="rounded-xl px-4 py-3 flex items-center justify-between gap-4 mb-4 transition-all"
      style={{
        border: `1px solid ${pulse ? "var(--stoa-accent)" : "var(--stoa-rule)"}`,
        background: pulse ? "color-mix(in oklab, var(--stoa-accent) 10%, var(--stoa-shine))" : "var(--stoa-shine)",
