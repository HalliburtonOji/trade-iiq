...
interface Trade {
  symbol: string; direction: string; entry_price: number; exit_price: number | null;
...
Respond in JSON: {"grade":"B","summary":"...","strengths":["..."],"improvements":["..."],"lesson":"..."}`;
      ...
      if (jsonMatch) {
        ...
        setReview({ grade: "B", summary: full.slice(0, 200), strengths: ["Trade was executed"], improvements: ["Set tighter stops"], lesson: "Risk Management" });
      }
    } catch {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[80] bg-background/80 backdrop-blur-sm flex items-center justify-center p-6">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md">
        <GlassCard className="border-primary/20">
          ...
          <Button onClick={onClose} className="w-full mt-4" size="sm">Done</Button>
        </GlassCard>
      </motion.div>
    </motion.div>
  );
